const Keyboard = {
	...
	init() {
		...
		onButton = document.getElementById("turnOnKeyboard")
		offButton = document.getElementById("turnOffKeyboard")
		onButton.addEventListener("click", () => {
		    this.properties.keyboardInputs.forEach((element) => {
				alert("Using this feature you can bypass text filters. However, you must also use the caesar cypher function otherwise the network can see your messages.")
		...
		offButton.addEventListener("click", () => {
		    this.properties.keyboardInputs.forEach((element) => {
		        this.close(element.value, (currentValue) => {
		            element.value = currentValue;
		        });
		    });
		});
			...
			switch (key) {
				...
				default:
					...
					this.keyElement.addEventListener("click", () => {
						...
						const rawChar = this.properties.capsLock
							? key.toUpperCase()
							: key.toLowerCase();

						const encrypted = this._caesarEncrypt(rawChar, 0);
						this.properties.value += encrypted;
						this._updateValueInTarget();
					});
					break;
			}
